...
function run_dice(
    ...
    @constraint(m, abateeq_r[ti in tidx,ri in ridx], ABATECOST_R[ti,ri] == YGROSS_R[ti,ri] * cost1tot[ti,ri] * (MIU_R[ti,ri]^expcost2[ti,ri]))
    @constraint(m, abateeq[ti in tidx], ABATECOST[ti] == sum(ABATECOST_R[ti,ri] for ri in ridx))

    # Carbon price equation from abatement
    @constraint(m, carbpriceeq_r[ti in tidx, ri in ridx], CPRICE_R[ti,ri] == pbacktime[ti,ri] * MIU_R[ti,ri]^(expcost2[ti,ri]-1))
    ...
    if(!is_solved_and_feasible(m; allow_almost=true))
        ...
        return((solved=false,status=status,m=m))
    end
